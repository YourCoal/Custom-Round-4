...
public class ACManager implements PluginMessageListener {
	...
	public static void sendChallenge(Player player) {
		...
		class SyncTask implements Runnable {
			...
			public void run() {
				Player player;
				try { player = CivGlobal.getPlayer(name);
				...
				} catch (CivException e) { CivLog.warning("Checking Player Failed");
				}
			}
					...
					if (resident != null && !resident.isUsesAntiCheat()) {
						if (player.isOp() || player.hasPermission(CivSettings.MINI_ADMIN)) {
							...
							TaskMaster.syncTask(new PlayerKickBan(player.getName(), true, false, "You must use AntiCheat to join this server."+
									"Visit http://leviathangamingmc.enjin.com/forum/ in 'CivCraft' to get it."));
						}
					}
				} catch (CivException e) {
				}
	...
	public void onPluginMessageReceived(String channel, Player player, byte[] messageRaw) {
		...
		if (iv == null) {
			BasicMessages.sendAError(player, "anticheat.ACManager-156: Invalid Auth Message(0xFFFFEEC1)");
			return;
		...
		try { Cipher cipher = Cipher.getInstance("DES/CBC/PKCS5Padding");
			...
			e1.printStackTrace();
			BasicMessages.sendAError(player, "anticheat.ACManager-169: Invalid Auth Message(0xFFFFEEA1)");
			return;
		} String decoded = "";
		...
		} try { validate(player, decoded);
			Resident resident = CivGlobal.getResident(player);
			if (resident != null) {
				resident.setUsesAntiCheat(true);
			} BasicMessages.sendSuccess(player, "You've been validated by CivCraft Anti-Cheat");
			return;
		} catch (CivException e) {
			BasicMessages.sendAError(player, "anticheat.ACManager-184: Couldn't Verify your client");
			BasicMessages.sendAError(player, e.getMessage());
