...
public class BuildCommand extends CommandBase {
	...
	public void validatenearest_cmd() throws CivException {
		...
		} BasicMessages.sendSuccess(player, "Running validation on "+buildable.getDisplayName()+" at "+buildable.getCenterLocation()+"...");
	...
	public void repairnearest_cmd() throws CivException {
		...
		if (War.isWarTime()) {
			...
			BasicMessages.send(player, CivColor.LightGreen+"Are you sure you want to repair the structure "+CivColor.Yellow+nearest.getDisplayName()+
			...
			BasicMessages.send(player, CivColor.LightGray+"If yes, use /build repairnearest yes");
			return;
		} town.repairStructure(nearest);		
		BasicMessages.sendSuccess(player, nearest.getDisplayName()+" repaired.");
	}
	...
	public void demolishnearest_cmd() throws CivException {
		...
		if (nearest == null) {
			...
			BasicMessages.send(player, CivColor.LightGreen+"Are you sure you want to demolish the structure "+CivColor.Yellow+nearest.getDisplayName()+
					CivColor.LightGreen+" at "+CivColor.Yellow+nearest.getCorner()+CivColor.LightGreen+" ?");
			BasicMessages.send(player, CivColor.LightGray+"If yes, use /build demolishnearest yes");
			nearest.flashStructureBlocks();
			return;
		} town.demolish(nearest, false);
		BasicMessages.sendSuccess(player, nearest.getDisplayName()+" at "+nearest.getCorner()+" demolished.");
	}
	
	public void demolish_cmd() throws CivException {
		Town town = getSelectedTown();
		if (args.length < 2) {
			BasicMessages.sendHeading(sender, "Demolish Structure");
			for (Structure struct : town.getStructures()) {
				BasicMessages.send(sender, struct.getDisplayName()+" type: "+CivColor.Yellow+struct.getCorner().toString()+
						CivColor.White+" to demolish");
			} return;
		} try { BlockCoord coord = new BlockCoord(args[1]);
			Structure struct = town.getStructure(coord);
			if (struct == null) {
				BasicMessages.sendAError(sender, "command.BuildCommand-108: No structure at "+args[1]);
				return;
			} struct.getTown().demolish(struct, false);
			BasicMessages.sendToTown(struct.getTown(), struct.getDisplayName()+" has been demolished.");
		} catch (NumberFormatException | ArrayIndexOutOfBoundsException e) {
			BasicMessages.sendAError(sender, "command.BuildCommand-113: Bad formatting. make sure to enter the text *exactly* as shown in yellow.");
		}
	}
	...
	public void progress_cmd() throws CivException {
		BasicMessages.sendHeading(sender, "Building Structures");
		Town town = getSelectedTown();
		for (BuildAsyncTask task : town.build_tasks) { Buildable b = task.buildable;
			DecimalFormat df = new DecimalFormat();
			BasicMessages.send(sender, CivColor.LightPurple+b.getDisplayName()+": "+CivColor.Yellow+"("+df.format(b.getBuiltHammers()) + "/"+b.getHammerCost()+")"+
	...
	public void list_available_structures() throws CivException {
		BasicMessages.sendHeading(sender, "Available Structures");
		Town town = getSelectedTown();
		for (ConfigBuildableInfo sinfo : CivSettings.structures.values()) {
			if (sinfo.isAvailable(town)) { String leftString = "";
				if (sinfo.limit == 0) {
					leftString = "Unlimited";
				} else {
					leftString = ""+(sinfo.limit - town.getStructureTypeCount(sinfo.id));
				} BasicMessages.send(sender, CivColor.LightPurple+sinfo.displayName+CivColor.Yellow+" Cost: "+sinfo.cost+
	...
	public void list_available_wonders() throws CivException {
		BasicMessages.sendHeading(sender, "Available Wonders");
		Town town = getSelectedTown();
		for (ConfigBuildableInfo sinfo : CivSettings.wonders.values()) {
			if (sinfo.isAvailable(town)) { String leftString = "";
				if (sinfo.limit == 0) {
					...
					BasicMessages.send(sender, CivColor.LightPurple+sinfo.displayName+
							CivColor.Yellow+
							" Cost: "+sinfo.cost+
							" Upkeep: "+sinfo.upkeep+" Hammers: "+sinfo.hammer_cost+ 
							" Left: "+leftString);
				} else { Wonder wonder = CivGlobal.getWonderByConfigId(sinfo.id);
				BasicMessages.send(sender, CivColor.LightGray+sinfo.displayName+" Cost: "+sinfo.cost+" - Already built in "+
	...
	public void preview_cmd() throws CivException {
		...
		} if (sinfo.isWonder) { Wonder wonder = Wonder.newWonder(getPlayer().getLocation(), sinfo.id, town);
			try { wonder.buildPlayerPreview(getPlayer(), getPlayer().getLocation());
			} catch (IOException e) {
				e.printStackTrace();
				throw new CivException("Internal IO Error.");
			}
		} else { Structure struct = Structure.newStructure(getPlayer().getLocation(), sinfo.id, town);
			try {
		...
		} BasicMessages.sendSuccess(sender, "Showing preview.");
	}
	...
	private void buildByName(String fullArgs) throws CivException {
		...
		if (sinfo == null) {
			...
			try { struct.buildPlayerPreview(getPlayer(), getPlayer().getLocation());
			} catch (IOException e) {
				e.printStackTrace();
				throw new CivException("Internal IO Error.");
			}
	...
	public void showHelp() {
		showBasicHelp();		
		BasicMessages.send(sender, CivColor.LightPurple+command+" "+CivColor.Yellow+"[structure name] "+
